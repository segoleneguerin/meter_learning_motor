...
% stimulus          = signal of the stimulus (only for plotting)
...
function [claps, peak_amp] = tap_corrector(timeVec, fs, stimulus, clapping_signal, claps, peak_amp, trial_dur, participant, group, condition, trial)
...
box off
title(['Participant ',num2str(participant),' - group ', num2str(group),' - condition ', num2str(condition),' - trial ', num2str(trial)])
